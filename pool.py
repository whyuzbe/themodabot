...
class DB:
    ...
    async def connect(self):
        if self.driver == "postgres":
            import asyncpg
            # Оптимизированный пул: min=5 (всегда наготове), max=25 (для пиковых нагрузок)
            self.pool = await asyncpg.create_pool(
                dsn=self.dsn_or_path,
                min_size=5,
                max_size=25,
                timeout=30.0,
    ...
    async def _migrate(self):
        ...
        # staff_accounts.ref_code / commission_pct
        try:
            if self.driver == "postgres":
                async with self.pool.acquire() as conn:
                    cols = await conn.fetch(
        ...
        # channel_posts.size / channel_posts.description
        try:
            if self.driver == "postgres":
                async with self.pool.acquire() as conn:
                    cols = await conn.fetch(
